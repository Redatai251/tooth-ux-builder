    ...
    <footer className="px-3 sm:px-6 pb-6 pt-16 bg-gradient-to-b from-brand-soft/40 to-secondary/40">
      <div className="mx-auto max-w-7xl">
          ...
          <div className="grid lg:grid-cols-12 gap-10">
                ...
                <div className="mt-3 flex flex-wrap gap-3">
                  {SITE.socials.map((s) => (
                    <a
                      ...
                      aria-label={s.name}
                      className="size-11 rounded-full overflow-hidden bg-white border border-border shadow-sm hover:shadow-md transition grid place-items-center"
                    >
                      <img
                        src={s.icon}
                        alt={s.name}
            ...
            <div className="lg:col-span-2">
              ...
              <ul className="mt-4 space-y-2 text-sm text-muted-foreground">
                {pages.map((p) => (
                  <li key={p.to}>
                    <Link to={p.to} className="hover:text-brand">
                      {p.label}
                    </Link>
                  </li>
                ))}
            ...
            <div className="lg:col-span-3">
              <h4 className="text-sm flex items-center gap-2">
                <Clock className="size-4 text-brand" /> Opening Hours
              </h4>
            ...
            <div className="lg:col-span-3">
              <h4 className="text-sm flex items-center gap-2">
                <Phone className="size-4 text-brand" /> Contact
              </h4>
              <ul className="mt-4 space-y-1.5 text-sm text-muted-foreground">
                {SITE.phones.map((p) => (
                  <li key={p}>
        ...
        <div className="mt-6 flex flex-wrap items-center justify-between gap-2 text-xs text-muted-foreground px-2">
          <p>
            Copyright © 2026 <span translate="no">Finan Speciality Dental Clinic</span>. All rights
            reserved.
          </p>
          <p>
